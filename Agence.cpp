...
void Agence::bien_txt(string path) {
    string const nomFic(path);
...
void Agence::client_txt(const string path, const int type) {
    string const nomFic(path);
    ifstream fic(nomFic.c_str());  // ouverture en lecture
    if (fic) {
        string line;
...
void Agence::creer_client() {
    ...
    if (choix == 2) {
        ...
        int type;
        cin >> type;
        cout << "Quel est le chemin absolu vers le fichier texte (nom du fichier inclus): " << endl;
        string path;
        cin >> path;
        client_txt(path, type);
    }
...
void Agence::ajout_appartement(Adresse _adresse, int _prix, int _m2, int ref) {
    ...
    int _nb_appart;
    cin >> _nb_appart;
    Appartement *nouveau_appartement = new Appartement(_adresse, _prix, _m2, ref, _pieces, _etage, _gar, _ca, _ba,
                                                       _nb_appart);
...
void Agence::recherche_biens() {
    ...
    cout
            << "Choix de filtres de recherche. Tappez 0 si vous ne désirez pas filtrer suivant le paramètre demandé -> "
            << endl;
    ...
    for (im = dico_biens[typebien].begin(); im != dico_biens[typebien].end(); im++) {
        ...
        if (((biencible->get_prix() <= choixprix) || (choixprix == 0)) &&
            ((biencible->get_m2() >= choixm2) || (choixm2 == 0))) {
...
void Agence::proposition_achat() {
    ...
    int refacheteur;
    cin >> refacheteur;
    cout << "La visite du bien (ref " << refbien
         << ") a été ajouté. Voulez-vous faire une proposition ? (1 : oui, 2 : non)" << endl;
    int prop;
    cin >> prop;
    int prixpropo;
    if (prop == 1) {
