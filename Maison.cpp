...
Maison::Maison(int _prix, int _m2, int _ref_client, int _piece, bool _garage, bool _jardin, bool _piscine) : Bien(_prix,
                                                                                                                  _m2,
                                                                                                                  _ref_client) {
    piece = _piece;
    garage = _garage;
...
Maison::Maison(Adresse _adresse, int _prix, int _m2, int _ref_client, int _piece, bool _garage, bool _jardin,
               bool _piscine) : Bien(_adresse, _prix,
                                     _m2,
                                     _ref_client) {
    piece = _piece;
    garage = _garage;
...
void Maison::recherche() {
    ...
    if ((piece <= choixpieces) || (choixpieces == 0)) {
        if (((choixgarage == 0) || ((choixgarage == 1) && (garage == true))) &&
            ((choixpiscine == 0) || ((choixpiscine == 1) && (piscine == true))) &&
            ((choixjardin == 0) || ((choixjardin == 1) && (jardin == true)))) {
            cout << "### 1. ###\n" << endl;
            affiche();
            cout << "\n" << endl;
        }
